...
var OWM_API_KEY = "b978aaf9be2002aab78f296d8798ca8e";
...
function saveSettings() {
  ...
  if (saved) {
	...
	if (saved.CalendarURL !== null) {
      if (saved.CalendarURL != config.CalendarURL) refreshCal = true;
    }  
  ...
  if (refreshCal) {
	if (DEBUG) console.log('Refreshing calendar after changing settings');  
	refreshCalendar();  
  }
...
function refreshCalendar() {
  if (config.CalendarURL) {
	// Trigger calendar fetch on command from Pebble
    if (DEBUG) console.log('Fetching calendar');
    fetchCalendar(config.CalendarURL);	
  }	else {
	if (DEBUG) console.log('No calendar URL');  
  }    
}
...
function fetchCalendar(url) {
  console.log('Loading calendar ' + url);
  download(url, function (err, data) {
    if (err) {
	  ...
	  if (event) {
		dict.push({key:"event",value:"event"});
	  }
	  var eventstart = localStorage.getItem('eventstart');
	  if (eventstart) {
		dict.push({key:"eventstart",value:"eventstart"});  
	  }	
	  Pebble.sendAppMessage(dict);	
    }    
    var ics = ical.parseICS(data);   
    //if (DEBUG) console.log('Found data ' + JSON.stringify(ics));
    var closest = findClosest (ics);
    if (DEBUG) console.log('Closest is ' + JSON.stringify(closest));
	  
	var closestTime = closest.start.toDateString();
	var hh = closest.start.getHours(); 
	if ((hh) && (hh>0)) {
		closestTime+=' '+hh+':'+closest.start.getMinutes();	
	}
	if (DEBUG) console.log('Closest time = ' + closestTime);  
	var closestDescription = closest.summary.trim();
	if (DEBUG) console.log('Closest summary = ' + closestDescription); 
	if (closest.location) {
		if (DEBUG) console.log('Closest location = ' + closest.location.trim()); 
		closestDescription+=' ('+closest.location.trim() + ')';
	}
	if (closest.description) {
		if (DEBUG) console.log('Closest location = ' + closest.description.trim()); 
		closestDescription+=' '+closest.description.trim();
	}  
	closestDescription = closestDescription.slice(0, 40);  
	  
	console.log('Event = ' + closestDescription);  
	localStorage.setItem('event', closestDescription);  
	localStorage.setItem('eventstart', closestTime);   
	Pebble.sendAppMessage({
            "eventstart":closestTime,
			"event":closestDescription
		    });  
  });	
}
